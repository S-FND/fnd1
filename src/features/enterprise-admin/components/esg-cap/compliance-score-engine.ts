...
export class ComplianceScoreEngine {
  ...
  constructor(config: Partial<EngineConfig> = {}) {
    this.config = {
      ...
      bufferPercent: config.bufferPercent ?? 0.25,
      today: config.today,
      juneCutoffFullCreditEnabled: config.juneCutoffFullCreditEnabled ?? true,
      juneCutoffMonth: config.juneCutoffMonth ?? 6,
      juneCutoffDay: config.juneCutoffDay ?? 30,
  ...
  private evaluateItem(raw: PlanItem, today: Date): ItemResult {
    ...
    const cutoffDate = targetForCutoff
      ...
      : null;
    // June cutoff status source is `raw.companyStatus` ONLY. No fallback to
    // investorStatus or status — investorStatus must have zero influence on
    // this decision, even if investorStatus is "closed".
    const companyStatus = (raw.companyStatus ?? "").toString().trim();
    const companyStatusLower = companyStatus.toLowerCase();
    const statusAllowsFullCredit = ["submitted", "closed"].includes(companyStatusLower);
    const juneCutoffApplied =
      this.config.juneCutoffFullCreditEnabled &&
      statusAllowsFullCredit &&
      !!targetForCutoff &&
    ...
    if (juneCutoffApplied && targetForCutoff) {
      completionDate = targetForCutoff;
      reasonCompletionDateSelected =
        `June cut-off rule: company status "${companyStatus}" and target date on or before June 30 — target date used as actual submission date (100% completion).`;
      businessRuleApplied.push(
        `June cut-off full-credit rule applied (company status ${companyStatus}, target date <= ${this.config.juneCutoffDay}/${this.config.juneCutoffMonth}).`,
      );
    }
