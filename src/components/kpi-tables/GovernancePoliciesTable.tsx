          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="w-[120px] text-center">In Place</TableHead>
                <TableHead className="w-[120px] text-center">Employee Training</TableHead>
                <TableHead className="w-[140px]">Last Update To The Policy (MM/YY)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={policy.id}>
                  ...
                  <TableCell className="text-center py-2">
                    <div className="flex items-center justify-center gap-2">
                      <span className="text-[10px] text-muted-foreground font-medium">{index + 1}a</span>
                      <Switch
                        checked={getBoolValue(policy.id, 'in_place')}
                        onCheckedChange={(checked) =>
                          handleBoolChange(policy.id, 'in_place', checked)
                        }
                        disabled={readOnly}
                      />
                      <span className="text-xs text-muted-foreground w-8">
                        {getBoolValue(policy.id, 'in_place') ? 'Yes' : 'No'}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell className="text-center py-2">
                    <div className="flex items-center justify-center gap-2">
                      <span className="text-[10px] text-muted-foreground font-medium">{index + 1}b</span>
                      <Switch
                        checked={getBoolValue(policy.id, 'training')}
                        onCheckedChange={(checked) =>
                          handleBoolChange(policy.id, 'training', checked)
                        }
                        disabled={readOnly}
                      />
                      <span className="text-xs text-muted-foreground w-8">
                        {getBoolValue(policy.id, 'training') ? 'Yes' : 'No'}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell className="py-2">
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-muted-foreground font-medium">{index + 1}c</span>
                      <Input
                        type="text"
                        placeholder="MM/YY"
                        maxLength={5}
                        value={getStringValue(policy.id, 'last_update')}
                        onChange={(e) => {
                          ...
                          handleStringChange(policy.id, 'last_update', value);
                        }}
