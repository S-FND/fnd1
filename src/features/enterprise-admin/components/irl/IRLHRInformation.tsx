...
const IRLHRInformation = ({ buttonEnabled }: { buttonEnabled: boolean }) => {
  ...
  // Define all possible HR questions with their keys
  const allHRQuestions = [
    { key: 'working_hours', name: 'Working hours for FTEs', type: 'input' },
    { key: 'shift_timing', name: 'Shift timing for contract workers', type: 'section' },
    { key: 'outsourced_services', name: 'Outsourced Services', type: 'section' },
    { key: 'facilities_list', name: 'List of major facilities', type: 'textarea' },
    { key: 'product_safety', name: 'Product safety certifications', type: 'textarea' },
    { key: 'emergency_incidents', name: 'Emergency incidents', type: 'textarea' },
    { key: 'employees_table', name: 'Human Resource Management - Employees', type: 'table' },
    { key: 'workers_table', name: 'Human Resource Management - Workers', type: 'table' },
    { key: 'differently_abled', name: 'Human Resource Management - Differently Abled Personnel', type: 'table' },
    { key: 'board_managerial', name: 'Key Managerial Positions / Board of Directors', type: 'table' },
    { key: 'retrenchment_details', name: 'Retrenchment details', type: 'textarea' }
  ];
  ...
  const fetchCompanyConfiguration = async (entityId: string) => {
    try {
      const response: any = await httpClient.get(
        `company-irl/${entityId}/irl-config?category=hr`
      );
  ...
  useEffect(() => {
    const loadConfiguration = async () => {
      ...
      try {
        ...
        logger.error('Error loading HR configuration:', err);
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        isDraft: true,
        entityId: entityId,
        workingHours: isQuestionEnabled('working_hours') ? formData.workingHours : '',
        shiftTiming: isQuestionEnabled('shift_timing') ? formData.shiftTiming : '',
        otHoursCurrent: isQuestionEnabled('shift_timing') ? formData.otHoursCurrent : '',
        otHoursPrevious: isQuestionEnabled('shift_timing') ? formData.otHoursPrevious : '',
        otPayCompensation: isQuestionEnabled('shift_timing') ? formData.otPayCompensation : '',
        facilitiesList: isQuestionEnabled('facilities_list') ? formData.facilitiesList : '',
        productSafetyCertifications: isQuestionEnabled('product_safety') ? formData.productSafetyCertifications : '',
        emergencyIncidents: isQuestionEnabled('emergency_incidents') ? formData.emergencyIncidents : '',
        retrenchmentDetails: isQuestionEnabled('retrenchment_details') ? formData.retrenchmentDetails : '',

        outsourcedServices: isQuestionEnabled('outsourced_services') ? outsourcedServices : [],

        hrFunction: isQuestionEnabled('employees_table') || isQuestionEnabled('workers_table') 
          ? employeeData.map((emp, index) => ({
              hr_management_function: emp.function,
              hr_management_emp_male: isQuestionEnabled('employees_table') ? emp.permanentMale : '',
              hr_management_emp_female: isQuestionEnabled('employees_table') ? emp.permanentFemale : '',
              other_hr_management_emp_male: isQuestionEnabled('employees_table') ? emp.otherMale : '',
              other_hr_management_emp_female: isQuestionEnabled('employees_table') ? emp.otherFemale : '',

              hr_management_workers_male: isQuestionEnabled('workers_table') ? (workerData[index]?.permanentMale || '') : '',
              hr_management_workers_female: isQuestionEnabled('workers_table') ? (workerData[index]?.permanentFemale || '') : '',
              other_hr_management_workers_male: isQuestionEnabled('workers_table') ? (workerData[index]?.otherMale || '') : '',
              other_hr_management_workers_female: isQuestionEnabled('workers_table') ? (workerData[index]?.otherFemale || '') : ''
            }))
          : [],

        hrmDifferentlyAbled: isQuestionEnabled('differently_abled') 
          ? differentlyAbledData.map(item => ({
        ...
        boardDirectors: isQuestionEnabled('board_managerial') ? boardDirectors : { male: '', female: '', total: '' },
        keyManagerial: isQuestionEnabled('board_managerial') ? keyManagerial : { male: '', female: '', total: '' },
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        outsourcedServices: isQuestionEnabled('outsourced_services') 
      ...
      <CardContent className="space-y-8">
        {isLoading || configLoading ? (
          ...
          <>
            ...
            {/* 7. Human Resource Management - Employees - Only show if enabled */}
            ...
            {/* 8. Human Resource Management - Workers - Only show if enabled */}
            ...
            {/* 9. Human Resource Management - Differently Abled Personnel - Only show if enabled */}
