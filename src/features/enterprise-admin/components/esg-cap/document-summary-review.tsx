...
type Props = {
  ...
  files: {
    ...
    reason?: string;                // ✅ add reason field (rejection reason)
    aiSummary: IDocumentValidation;
  }[];
  onClose: () => void;
  // onSubmit?: ... (commented for now)
};
...
export default function DocumentSummaryDialog({
  ...
  // ✅ Load status & reason from the currently selected file
  useEffect(() => {
    if (open && files.length > 0) {
      ...
      setReason(file.reason || "");   // load existing rejection reason
    }
  }, [open, selectedIndex, files]);

  // ✅ Reset when dialog closes (optional)
  useEffect(() => {
    if (!open) {
      setStatus(null);
      setReason("");
  ...
  const handleSubmit = () => {
    ...
    // onSubmit will be uncommented later
  };
