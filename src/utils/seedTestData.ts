...
VALUES ('Test Company', 'test-company', true, 'approved');
    ...
    scope1Templates.slice(0, 3).forEach((template, index) => {
      ...
      const dataKey = `scope1_data_collections_${template._id}_${currentMonth}_${currentYear}`;
      localStorage.setItem(dataKey, JSON.stringify([sampleData]));
      localStorage.setItem(`scope1_status_${template._id}_${currentMonth}_${currentYear}`, 'Draft');
    });
