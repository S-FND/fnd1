          ...
          <div className="overflow-x-auto">
            ...
            <TableHeader>
              <TableRow className="bg-muted/50">
                <TableHead className="w-[140px]">Subcategory</TableHead>
                <TableHead className="w-[80px]">Level</TableHead>
                <TableHead className="w-[60px] text-center">N/A</TableHead>
                <TableHead className="w-[70px]">Number</TableHead>
                <TableHead className="w-[180px]">Certificate Names</TableHead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-center py-2">
                        <Checkbox
                          checked={rowIsNA}
                          onCheckedChange={(checked) => handleNAChange(row.subcategory, level, !!checked)}
                          disabled={readOnly}
                      ...
                      <TableCell className="py-2">
                        <Input
                          ...
                          value={getValue(row.subcategory, level, 'number')}
                          onChange={(e) =>
                            handleChange(row.subcategory, level, 'number', e.target.value)
                          }
                      ...
                      <TableCell className="py-2">
                        <Input
                          placeholder={rowIsNA ? '-' : 'Certificate names'}
                          value={getValue(row.subcategory, level, 'names')}
                          onChange={(e) =>
                            handleChange(row.subcategory, level, 'names', e.target.value)
                          }
                      ...
                      <TableCell className="py-2">
                        <Input
                          placeholder={rowIsNA ? '-' : 'Validity'}
                          value={getValue(row.subcategory, level, 'validity')}
                          onChange={(e) =>
                            handleChange(row.subcategory, level, 'validity', e.target.value)
                          }
                      ...
                      <TableCell className="py-2">
                        <Input
                          placeholder={rowIsNA ? '-' : 'Comments'}
                          value={getValue(row.subcategory, level, 'comments')}
                          onChange={(e) =>
                            handleChange(row.subcategory, level, 'comments', e.target.value)
                          }
                          className="h-8 text-sm"
                          disabled={readOnly || rowIsNA}
                        />
