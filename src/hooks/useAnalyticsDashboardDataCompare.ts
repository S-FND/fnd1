...
export const useAnalyticsDashboardDataCompare = (filters: AnalyticsFilters, kpiEntries?: { companyId: string; kpi_id: string; value: string | null; quarter: string; year: number }[],
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (filters.year && filters.year == 2025) {
          filteredCompanies = filteredCompanies.filter(c => !['company-222','company-223','company-224'].includes(c.id))
        }
