...
export interface UploadedDocument {
  ...
  filename: string;
  s3Link?: string;
  signedUrl?: string;        // <-- Pre-signed URL from backend
  fileUrl?: string;
  uploadedBy?: string;
...
const confirmDeleteDocument = async () => {
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      validationDocId: doc.id,   // assuming doc.id is the validationDocId
    });
  ...
  // --- DOWNLOAD: Use signedUrl to download file ---
  const handleDownload = async (doc: UploadedDocument) => {
    ...
    try {
      ...
      a.href = blobUrl;
      a.download = doc.filename; // This sets the downloaded filename
