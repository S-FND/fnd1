...
// Compliance Rating Function - Complete with all grades
const getComplianceRating = (score: number = 0) => {
  if (score >= 85) {
    return { grade: "AA", label: "On Track", color: "text-green-600", bgColor: "bg-green-100", minScore: 85 };
  }
  if (score >= 70) {
    return { grade: "A", label: "Stable", color: "text-emerald-600", bgColor: "bg-emerald-100", minScore: 70 };
  }
  if (score >= 55) {
    return { grade: "BB", label: "Needs Attention", color: "text-yellow-600", bgColor: "bg-yellow-100", minScore: 55 };
  }
  if (score >= 40) {
    return { grade: "B", label: "At Risk", color: "text-orange-600", bgColor: "bg-orange-100", minScore: 40 };
  }
  // Below 40% - Grade C (Critical)
  return { grade: "C", label: "Critical", color: "text-red-600", bgColor: "bg-red-100", minScore: 0 };
};

// ✅ Single consistent color - matches your UI theme
const CHART_COLOR = '#10b77f'; // Indigo - matches primary color
const CHART_GRADIENT = '#818cf8'; // Lighter indigo for gradient
...
export const ComplianceGraphModal: React.FC<ComplianceGraphModalProps> = ({
  ...
  // Fetch data when modal opens or entityId changes
  useEffect(() => {
    if (open && entityId) {
  ...
  // ✅ Determine series keys with SINGLE consistent color
  const seriesKeys = useMemo(() => {
    if (selectedYear === 'all') {
      ...
      return sortedYears.map((year, index) => ({
        key: `${year}`,
        name: `${year}`,
        color: CHART_COLOR, // ✅ Single consistent color
      }));
    } else {
  ...
  // Custom Y-axis tick formatter to show ALL grades (AA, A, BB, B, C)
          ...
          {score !== null && score !== undefined && (
            <div className="mt-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Badge className={`${rating?.bgColor} ${rating?.color} border-0 text-xs`}>
                  {rating?.label}
                </Badge>
              </div>
            </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl">
            ...
            {currentScore !== null && (
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <span className={`text-2xl font-bold ${currentRating?.color}`}>
                    {currentRating?.grade}
                  </span>
                ...
                <Badge className={`${currentRating?.bgColor} ${currentRating?.color} border-0`}>
                  {currentRating?.label}
                </Badge>
              </div>
            )}
        ...
        {/* Quarterly Grade Cards - AA, A, BB, B, C */}
            ...
            {getQuarterlyData.map((q, index) => (
              <Card key={index} className={`text-center p-3 ${q.bgColor || 'bg-gray-50'} border-0 shadow-sm`}>
                <CardContent className="p-0">
                  ...
                  <div className={`text-2xl font-bold ${q.color || 'text-gray-400'}`}>
                    {q.grade || '—'}
                  </div>
                  <div className="text-xs font-medium text-muted-foreground">
                    {q.score !== null ? `${q.score}%` : 'No data'}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        {error ? (
          ...
          <div className="h-64 flex items-center justify-center text-muted-foreground">
            No data available. Compliance scores will appear after the first monthly snapshot.
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={350}>
            {chartType === 'line' ? (
              <LineChart data={chartData} margin={{ top: 20, right: 30, left: 30, bottom: 5 }}>
                ...
                <XAxis 
                  dataKey={viewType === 'quarterly' ? 'quarter' : 'month'} 
                  tick={{ fontSize: 12 }}
                />
                <YAxis 
                  domain={[0, 100]} 
        ...
        <div className="text-xs text-muted-foreground text-center mt-2 border-t pt-2">
          Illustrative (tracked quarter on quarter)
        </div>
      </DialogContent>
    </Dialog>
  );
