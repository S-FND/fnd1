...
const ESGCapDetailsPage: React.FC = () => {
    ...
    const all = useMemo(() => [...mockESGCapItems], []);
    const item = {
        "reportId": "66b2184decc90de1be26a79c",
        "id": "1778131191053-0.8199473549685693",
        "item": "Risk Management",
        "measures": "Conduct risk assessments at each centre to identify potential hazards and implement preventive measures and mitigation protocols.",
        "category": "governance",
        "priority": "Medium",
        "issue": "No risk assessments conducted at any centre.",
        "relatedFinding": "At present, no risk assessments conducted at each centre; SFL plans to initiate going forward. Source: DD Report (SAGE Sustainability, 22-Aug-2025), p.29.",
        "resource": "Operations / Compliance",
        "deliverable": "Approved SOP + sign-off from leadership + comms record",
        "timelineMonth": null,
        "targetDate": "6/2/2026",
        "actualDate": "",
        "status": "pending",
        "dealCondition": "CS",
        "esgLever": "Governance & Responsible Investment",
        "capSource": "ESG DD",
        "lastReviewDate": "4/7/2026",
        "implementationSupportNeeded": "Fireside guidance + External partner (H&S consultant)",
        "closureVerifiedBy": "Fireside ESG Team",
        "createdAt": "2026-05-07T05:19:51.053Z"
    }
    //   all.find((i) => i.id === id);
    ...
    const priorityTone = item?.priority === 'high' ? 'red' : item?.priority === 'medium' ? 'amber' : 'green';
                ...
                <div className="mx-auto max-w-[1440px] px-6 py-8 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight">{item?.issue}</h1>
                                {/* <p className="mt-1 max-w-3xl text-sm text-muted-foreground">{item?.description}</p> */}
                                <div className="mt-4 flex flex-wrap items-center gap-2">
                                    <MetaPill label={item.dealCondition} tone="slate" />
                                    <MetaPill
                                        label={`${item?.priority.charAt(0).toUpperCase() + item?.priority.slice(1)} Priority`}
                                        tone={priorityTone as any}
                                    />
                                    <MetaPill label={`Due ${new Date(item?.targetDate).toLocaleDateString()}`} tone="blue" />
                                    <MetaPill label={item?.category.charAt(0).toUpperCase() + item?.category.slice(1)} tone="default" />
                                    <MetaPill label={item?.status.replace('_', ' ')} tone={statusToneMap[item?.status] ?? 'slate'} />
                                </div>
                            </div>
                    ...
                    <SectionCard title="Reference Details" subtitle="Finding context and corrective measures" icon={<Info className="h-4 w-4" />}>
                        <div className="space-y-6">
                            <Field label="Issue & Related Finding" value={item.issue} />
                            <Field label="Measures & Corrective Actions" value={item.measures} />
                        </div>
                    </SectionCard>
                    ...
                    <SectionCard title="Completion Tracking" subtitle="Milestones and required artefacts" icon={<CheckCircle2 className="h-4 w-4" />}>
                        <div className="grid gap-8 lg:grid-cols-2">
                            <div>
                                ...
                                <ul className="mt-4 space-y-3">
                                    {item.deliverable.split("##").map((label) => (
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                            ...
                            <Field label="Target Date" value={new Date(item.targetDate).toLocaleDateString()} />
                            <Field label="Actual Completion" value={item.actualDate ? new Date(item.actualDate).toLocaleDateString() : 'Pending'} />
                            <Field label="Last Review Date" value="Aug 05, 2026" />
                            <Field label="Closure Verified By" value="Pending" />
                        </div>
                    </SectionCard>
