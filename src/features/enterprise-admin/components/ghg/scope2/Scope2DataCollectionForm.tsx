...
export const Scope2DataCollectionForm = () => {
  ...
  const getTemplateData = async (templateId: string) => {
    try {
      let response = await httpClient.get(`ghg-accounting/${templateId}/ghg-data-collection?financialYear=${selectedYear}&scope=Scope2`);
    ...
    <UnifiedSidebarLayout>
      <div className="container mx-auto py-6 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Data Collection - Scope 2</h1>
              ...
              <div key={entry.id} className="border rounded-lg p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={`${((entry.activityDataValue || 0) * (template.emissionFactor || 0) / 1000).toFixed(3)} tCO₂e`}
                      disabled
                      className="bg-muted"
                    />
                  </div>
