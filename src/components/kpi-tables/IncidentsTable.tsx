              ...
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="w-[140px]">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs font-normal text-muted-foreground italic">(For the Quarter)</span>
                  </div>
                </TableHead>
                <TableHead className="w-[160px]">
                  <div className="flex flex-col">
                    <span>Cases Open/Unresolved</span>
                    <span className="text-xs font-normal text-muted-foreground italic">(As of Date)</span>
                  </div>
                </TableHead>
                <TableHead className="w-[140px]">
                  <div className="flex flex-col">
                    <span>Impact on Business</span>
                    <span className="text-xs font-normal text-muted-foreground italic">(High/Medium/Low)</span>
                  </div>
                </TableHead>
              </TableRow>
            </TableHeader>
                ...
                <TableRow key={category.id}>
                    ...
                    <div className="flex items-start gap-1">
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Info className="w-3.5 h-3.5 text-muted-foreground cursor-help flex-shrink-0 mt-0.5" />
                        </TooltipTrigger>
                        <TooltipContent side="right" className="max-w-xs bg-popover border border-border z-50">
                  ...
                  <TableCell className="py-2">
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-muted-foreground font-medium">{kpiNumber}a</span>
                      <Input
                        type="number"
                        placeholder="0"
                        min="0"
                        value={getValue(category.id, 'cases')}
                        onChange={(e) => handleChange(category.id, 'cases', e.target.value)}
                  ...
                  <TableCell className="py-2">
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-muted-foreground font-medium">{kpiNumber}b</span>
                      <Input
                        type="number"
                        placeholder="0"
                        min="0"
                        value={getValue(category.id, 'open_cases')}
                        onChange={(e) => handleChange(category.id, 'open_cases', e.target.value)}
                        className="w-20 h-8 text-sm"
                        disabled={readOnly}
                      />
                    </div>
                  ...
                  <TableCell className="py-2">
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-muted-foreground font-medium">{kpiNumber}c</span>
                      <Select
                        key={`impact-${category.id}-${getValue(category.id, 'impact') ?? 'empty'}`}
                        value={getValue(category.id, 'impact')}
                        onValueChange={(value) => handleChange(category.id, 'impact', value === '__clear__' ? undefined : value)}
                        disabled={readOnly}
                      >
